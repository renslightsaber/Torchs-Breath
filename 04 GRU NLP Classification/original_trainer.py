...
def train_one_epoch(model, dataloader, loss_fn, optimizer, device, epoch, n_classes =None, scheduler = None, grad_clipping = False):
    ...
    for data in bar:
        x = data[0].to(device) # [bs, sl]

        bs = x.shape[0] # bs

        y_true = data[1].to(device) # [bs]
        y_pred = model(x)           # [bs, 4]
        ...
        preds.append(y_pred) # preds: [[bs, 4], [bs, 4], [bs, 4],  ... ]
        trues.append(y_true) # trues: [[bs], [bs], ... ]
        
        preds_cat = torch.cat(preds, dim = 0) # [total_bs, 4]
        trues_cat = torch.cat(trues, dim = 0) # [total_bs]
...
# Run Train
def run_train(model, train_loader, valid_loader, loss_fn, optimizer, device, n_classes=None, scheduler = None, grad_clipping = False, 
              n_epochs = 30, print_iter = 10, early_stop = 20):
    ...
    for epoch in range(n_epochs):
        ...
        if valid_loss < lowest_loss:
            ...
            lowest_epoch = epoch
            torch.save(model.state_dict(), './model_nlp_gru.bin')
        else:
    ...
    # model load
    model.load_state_dict(torch.load('./model_nlp_gru.bin'))
